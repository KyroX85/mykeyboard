...
function updateKillerFeatureIfNeeded(existing, details = {}) {
  if (details && details.killerFeatureScore) {
    return updateKillerFeatureMemory(existing, details.killerFeatureScore);
  }
  const feature = details && (details.feature || details.idea || details.proposal || details.founderMessage || details.agentAnswer);
...
function updateFounderMentalStateIfNeeded(existing, details = {}) {
  ...
  const estimate = estimateFounderMentalState(message, {
    ...details,
    founderMentalStateMemory: existing,
    previousMentalState: existing && existing.lastState
  });
...
function updateEvidenceRequirementIfNeeded(existing, details = {}) {
  if (details && details.evidenceRequirementCheck) {
    return updateEvidenceRequirementMemory(existing, details.evidenceRequirementCheck);
  }
  const claim = details && (details.agentAnswer || details.response || details.claim);
