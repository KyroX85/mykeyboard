function generatePassiveWorkerUpdates(state, sources = {}) {
  ...
  if (Array.isArray(execution.completed) && execution.completed.length > 0) {
    updates.push(`Founder, ${owner(execution.completed[0])} completed ${execution.completed[0].action} with rollback notes recorded.`);
  }

  if (Array.isArray(execution.dryRun) && execution.dryRun.length > 0) {
    updates.push(`Founder, ${owner(execution.dryRun[0])} paused at dry-run for ${execution.dryRun[0].action}. No risky execution done.`);
  }

  if (Array.isArray(execution.blocked) && execution.blocked.length > 0) {
    updates.push(`Founder, ${owner(execution.blocked[0])} blocked ${execution.blocked[0].action}; guardrails stayed active.`);
  }

  if (Array.isArray(execution.rolledBack) && execution.rolledBack.length > 0) {
    updates.push(`Founder, ${owner(execution.rolledBack[0])} needs rollback review for ${execution.rolledBack[0].action}.`);
  }

  if (Array.isArray(maintenance.blocked) && maintenance.blocked.length > 0) {
    updates.push(`Founder, Auditor is still blocking ${maintenance.blocked[0].action}; not safe enough yet.`);
  }

  const sections = (state && state.sections) || {};
  const topRisk = first(sections.risks) || first(sections.unresolved);
  if (topRisk) {
    updates.push(`Founder, Reviewer still sees risk: ${compact(topRisk)}.`);
  }
