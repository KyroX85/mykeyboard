...
class InteractionRhythmGuardrailsTest {
    ...
    fun suggestionsResolveOffMainThreadAndDiscardStaleResults() {
        ...
        assertTrue(source.contains("autocorrectExecutor"))
        assertTrue(updateSuggestions.contains("autocorrectPrefetchFuture?.cancel(true)"))
        assertTrue(updateSuggestions.contains("suggestionExecutor.submit"))
        assertTrue(updateSuggestions.contains("predictor.getSuggestions(prefix, previousWord)"))
        assertFalse(updateSuggestions.contains("predictor.getAutocorrection(prefix, previousWord)"))
        assertTrue(updateSuggestions.contains("prefetchAutocorrection(prefix, previousWord, requestGeneration)"))
